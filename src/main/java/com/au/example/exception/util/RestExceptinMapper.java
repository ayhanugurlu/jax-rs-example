...
public class RestExceptinMapper implements ExceptionMapper<ServerException> {
	...
	public Response toResponse(ServerException exception) {
		...
		return Response.status(500).entity(exception).header("ErrorMessage", errorMessage).header("ExceptionClasses", exceptionClasses)
				.header("PossibleCauses", possibleCauses).build();
	}
	...
	private String possibleCauses(Exception exception) {
		...
		for (StackTraceElement stackTraceElement : parentException.getStackTrace()) {
			if (stackTraceElement.getClassName().startsWith(packageName)) {
				possibleCauses += stackTraceElement.getClassName() + "|" + stackTraceElement.getFileName() + "|" + stackTraceElement.getMethodName()
	...
	private String exceptionClasses(Throwable exception) {
		return (exception == null) ? "" : ((exception != exception.getCause()) ? exception.getClass().getName() + "|"
				+ exceptionClasses(exception.getCause()) : "");
	}

	private String collectMessages(Throwable exception) {
		return (exception.getMessage() == null ? "" : exception.getMessage())
				+ (exception.getCause() != null ? "|" + collectMessages(exception.getCause()) : "");
	}
